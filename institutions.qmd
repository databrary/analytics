...
n_institutions <- length(unique(inst_df$id))
```
...
inst_affils_df <- dplyr::left_join(inst_df, inst_invest_df, by = "id") |>
  dplyr::group_by(id, name) |>
  dplyr::summarise(n_ais = n()) |>
  dplyr::arrange(desc(n_ais)) |>
  dplyr::mutate(name = paste0("<a href='https://databrary.org/institution/", id, "'>", name, "</a>"))
```
...
#| fig-cap: "Histogram of percent of Investigators at an Institution who have created publicly visible (*Shared* or *Overview Only*) volumes."
inst_invest |>
  dplyr::mutate(p_vols = round(100*n_ais_w_vols/n_ais, 1)) |>
  dplyr::select(p_vols) |>
  ggplot() +

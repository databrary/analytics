...
db_stats_old <- readr::read_csv("csv/institutions-investigators.csv")
n_rows <- dim(db_stats_old)[1]

updated_stats <- rbind(db_stats_old, db_stats_new[,1:4])
readr::write_csv(updated_stats, file = "csv/institutions-investigators.csv")
```
...
users_plot <- gathered_stats |>
  ...
  ylab("Authorizations") +
  databrary_theme +
  scale_y_continuous(breaks = c(0, 300, 600, 900, 1200, 1500, 1800, 2100)) +
...
::: {.callout-warning}
As of 2026-02-04, the Google Scholar API has added CAPTCHA, probably to prevent automated mining of the sort we're doing here.

So, the following chunks have `eval: false` added.
:::

```{r}
#| label: weekly-get-old-citation-data
#| eval: false
...
```{r}
#| label: weekly-merge-old-cite-new
#| eval: false
...
```{r , }
#| label: fig-db-dv-citations-plot
#| fig-cap: "Change in citations of Databrary and Datavyu over time"
#| eval: false
#| 

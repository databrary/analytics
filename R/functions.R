...
get_volume_ss_save_csv <-
  function(vol_id = 4,
    ...
    if (is.null(c)) {
      if (vb)
        message(" No text data returned from ", this_url)
      return(NULL)
    }
    
    df <-
      readr::read_csv(c, col_types = "cccccccccccccccccccccccccccccc")
    if (is.data.frame(df)) {
      if (vb)
...
get_volume_data <-
  function(vol_id = 1,
    ...
    vol_data <- databraryr::list_containers_records(vol_id)
    if (is.null(vol_data)) {
      if (vb)
